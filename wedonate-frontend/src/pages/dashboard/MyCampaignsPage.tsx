...
import { Target, Plus, ArrowRight, Calendar } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className={cn('text-2xl font-extrabold', isDark ? 'text-white' : 'text-gray-900')}>My Campaigns</h1>
          <p className={cn('text-sm mt-1', isDark ? 'text-slate-400' : 'text-gray-500')}>
            Track your fundraising campaigns
          </p>
        </div>
        <Link to="/donate">
          <Button size="sm" leftIcon={<Plus className="w-4 h-4" />}>New Campaign</Button>
        </Link>
      </div>
      ...
      {isLoading ? (
        ...
        <Card className="text-center py-16">
          ...
          <p className={cn('font-medium', isDark ? 'text-slate-400' : 'text-gray-400')}>No campaigns yet</p>
          <Link to="/donate">
              ...
              <Card key={camp.id} className="p-5">
                ...
                <div className="mb-3">
                  <div className="flex justify-between text-xs mb-1 font-medium">
                    ...
                    <span className={isDark ? 'text-slate-400' : 'text-gray-500'}>{Math.round(pct)}% of {formatCurrency(camp.goalAmount)}</span>
                  </div>
                ...
                {camp.adminNote && (
                  <div className={cn('mt-3 text-xs px-3 py-2 rounded-lg',
                    ...
                    <span className="font-semibold">Admin note:</span> {camp.adminNote}
                  </div>
                )}
              </Card>
            );
